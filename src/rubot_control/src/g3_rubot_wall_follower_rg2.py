...
def clbk_laser(msg):
    ...
    # if not isScanRangesLengthCorrectionFactorCalculated == True:
    N = int(len(msg.ranges))
    scanRangesLengthCorrectionFactor = N/360   #N = numero de laser beams
...
def take_action(regions):
    ...
    if regions['front'] > d and regions['fright'] > 2*d and regions['right'] > 2*d and regions['bright'] > 2*d:
        ...
        linear_x = vx
        angular_z = 0
    elif regions['front'] < 1.3*d:
        state_description = 'case 2 - front'
        linear_x = 0
        angular_z = wz
    elif regions['fright'] < 0.75*d:
        state_description = 'case 3 - fright'
        linear_x = 0
        angular_z = wz
    elif regions['front'] > 1.3*d and regions['right'] < d:
        state_description = 'case 4 - right'
        linear_x = vx
        angular_z = 0
    elif regions['bright'] < 0.9*d:
        ...
        linear_x = vx*0.5   #poser s'ha de canviar
        angular_z = -wz
